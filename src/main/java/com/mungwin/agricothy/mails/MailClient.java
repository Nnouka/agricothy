package com.mungwin.agricothy.mails;


import com.mungwin.agricothy.constants.ApiConstants;
import com.mungwin.agricothy.domain.dtos.data.EmailAddress;
import com.mungwin.agricothy.domain.dtos.data.MailCtxDto;
...
public class MailClient {
  ...
  public void sendVoucherMail(final EmailAddress recipient,
                              final String fromAddress,
                              final String subject,
                              final Locale locale,
                              List<MailCtxDto> mailCtxList) {
    sendMails(Collections.singletonList(recipient), fromAddress, subject, locale, mailCtxList, EMAIL_VOUCHER_TEMPLATE);
  }
  public void sendVoucherRequestMail(final List<EmailAddress> recipients,
                              final String fromAddress,
                              final String subject,
                              final Locale locale,
                              List<MailCtxDto> mailCtxList) {
    sendMails(recipients, fromAddress, subject, locale, mailCtxList, EMAIL_VOUCHER_REQUEST_TEMPLATE);
  }
  public void sendPaymentDetails(final EmailAddress recipient,
                              final String fromAddress,
                              final String subject,
                              final Locale locale,
                              List<MailCtxDto> mailCtxList) {
    sendMails(Collections.singletonList(recipient), fromAddress, subject, locale, mailCtxList, EMAIL_PAYMENT_DETAIL_TEMPLATE);
  }
  ...
  public void sendResetPasswordMail(
    ...
    public void sendReferralMail(
        ...
        sendMails(recipients, fromAddress, subject, locale, mailCtxList, EMAIL_REFERRAL_TEMPLATE);
        /*recipients.forEach(
                emailAddress -> {
                    try {
                        sendInfoMail(
                                emailAddress, fromAddress,
                                subject, locale, EMAIL_REFERRAL_TEMPLATE, mailCtxList
                        );
